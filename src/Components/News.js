...
export class News extends Component {
    ...
    async componentDidMount() {
        let url = "https://newsapi.org/v2/top-headlines?country=in&apiKey=153e324c9b5847f58a17aae8fe1569a2&page=1&pageSize=12";
        let data = await fetch(url);
        let parsedData = await data.json();
        console.log(parsedData);
        this.setState({ articles: parsedData.articles, totalResults : parsedData.totalResults , totalResults : parsedData.totalResults })
    }
    ...
    handleNextClick = async () => {
       this.setState({
        ...
        let url = `https://newsapi.org/v2/top-headlines?country=in&apiKey=153e324c9b5847f58a17aae8fe1569a2&page=${this.state.page+1}&pageSize=12`;
        let data = await fetch(url);
        let parsedData = await data.json();
        console.log(parsedData);
        ...
        return (
            <div className="container my-3" >
                ...
                <div class="d-flex justify-content-between">
                    ...
                    <button disabled={this.state.page+1>Math.ceil(this.state.totalResults/12)} type="button" class="btn btn-dark" onClick={this.handleNextClick}>Next&rarr;</button>
                </div>
            </div>
        )
    }
